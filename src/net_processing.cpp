                                         ...
                                         const CChainParams& chainparams, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!ProcessNetBlockHeaders(m_chainman, pfrom, {cmpctblock.header}, state, chainparams, &pindex)) {
            if (state.IsInvalid()) {
        ...
        if (fBlockReconstructed) {
            ...
            ProcessNetBlock(m_chainman, chainparams, pblock, /*fForceProcessing=*/true, &fNewBlock, pfrom, m_connman);
            if (fNewBlock) {
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool fNewBlock = false;
        ProcessNetBlock(m_chainman, chainparams, pblock, forceProcessing, &fNewBlock, pfrom, m_connman);
        if (fNewBlock) {
            pfrom.nLastBlockTime = GetTime();
        } else {
            LOCK(cs_main);
